...
```python
mro(A) = [A] + merge( mro(B), mro(C), [B,C] )  # 这里面只展示的是a的继承关系
# C3算法，先拿merge里面的第一个【】里面的第一个值，与merge里面其他的【】除第一位的值作比较，如果没有相同的，提取出来， 然后剔除掉改位
mro(A) = [A] + merge( [B, object], [C, object], [B, C] )
# 1， 先取B，比较【c, object】，【b，c】里面的object 和 c对比，发现没有相同的，提出B，剔除所有首位是b的  ##merge( [object], [c, object], [c] )
# 2， 取object，发现【c, object】[1:]有相同的，不提出
# 3， 取【c, object】中的c，发现与【c】[1:]中没有相同的，先提出，再剔除   ##merge( [object], [object], [] )
# 4， 取【c, object】中的object，发现没有相同的，先提出，再剔除   ##merge( [object], [object], [] )
mro(A) = [A] + [B,C,object]
mro(A) = [A,B,C,object]
```
...
mro(a) = [a] + merge( [b, object], [c, d, object], [b, c])
# 1， 取[b, object]中的b，找[c, d, object][1: ], [b, c][1: ],发现没有相同的，提出，剔除  ##merge( [object], [c, d, object], [c])
# 2， 取[object]的object，对应有相同的，过
# 3， 取[c, d, object]中的c，对应后面无相同，提出，剔除  ##merge( [object], [d, object], [])
# 4， 取[d, object]中的d，无相同，提出，剔除  ##merge( [object], [object], [])，
# 5， 最后对应object
...
```python
mro(a) = [a] + merge( mro(b), mro(c), [b,c])
mro(a) = [a] + merge( [b, d, object], [c, object], [b, c] )
# 1，取[b, d, object]中的b, 找[c, object][1:], [b, c][1:]，无相同，提剔  ## merge( [d, object], [c, object], [c] )
# 2，取[d, object]中的d，找[c, object][1: ], [c][1: ]，无相同，提剔  ## merge( [object], [c, object], [c] )
# 3，取[object]的object，对应有相同的，过
# 4，取[c, object]中c，无相同，提剔  ## merge( [object], [object], [] )
...
#  b                    ## merge( [d, object], [c, d, object], [c])                         
#  d
#  object
#  c                    ## merge( [d, object], [d, object], []) 
#  d                    ## merge( [object], [object], [])   
#  object

mro(a) = [a] + merge( [b, c, d, object] )
mro(a) = [a, b, c, d, object] 
```
...
mro(d) = [d] + merge( mro(g), mro(h), [g, h] )  ==> [d, g, h, k]
mro(e) = [e, m]
mro(f) = [f, m, n]

mro(b) = [b] + merge( [d, g, h, k], [e, m], [d, e] )  ==> [b, d, g, h, k, e, m]
mro(c) = [c] + merge( [e, m], [f, m, n], [e, f] )  ==> [c, e, f, m, n]
mro(a) = [a] + merge( [b, d, g, h, k, m, n, e, f], [c, e, f, m, n], [p], [b, c, p] )
# b  ##merge( [d, g, h, k, e, m], [c, e, f, m, n], [p], [c, p] )
# d  ##merge( [g, h, k, e, m], [c, e, f, m, n], [p], [c, p] )
# g  ##merge( [h, k, e, m], [c, e, f, m, n], [p], [c, p] )
# h  ##merge( [k, e, m], [c, e, f, m, n], [p], [c, p] )
# k  ##merge( [e, m], [c, e, f, m, n], [p], [c, p] )
# e
# m
# c  ##merge( [e, m], [e, f, m, n], [p], [p] )
# e  ##merge( [m], [f, m, n], [p], [p] )
# f  ##merge( [m], [m, n], [p], [p] )
# m  ##merge( [], [n], [p], [p] )
# n  ##merge( [], [], [p], [p] )
