...
HTTP协议是Hyper Text Transfer Protocol（超文本传输协议）的缩写,是用于万维网（WWW:World Wide Web ）服务器与本地浏览器之间传输超文本的传送协议。HTTP是一个属于**应用层**的面向对象的协议，由于其简捷、快速的方式，适用于分布式超媒体信息系统。它于1990年提出，经过几年的使用与发展，得到不断地完善和扩展。HTTP协议工作于客户端-服务端架构为上。浏览器作为HTTP客户端通过URL向HTTP服务端即WEB服务器发送所有请求。Web服务器根据接收到的请求后，向客户端发送响应信息。
...
请求头与请求体(内容)之间有一行空值，用两个`\r\n`表示

> 请求方式:  get 与 post 请求
>
> - GET提交的数据会放在URL之后，以?分割URL和传输数据，参数之间以&相连，如EditBook?name=test1&id=123456. POST方法是把提交的数据放在HTTP包的请求体中.
> - GET提交的数据大小有限制（因为浏览器对URL的长度有限制），而POST方法提交的数据没有限制
> - 根本区别：数据存放地点不同，get 放到 url 中（在URL“ /form/entry ”后面+[？数据]），post 放到 请求体中（内容实体之中）
...
```html
<标签名 属性1=“属性值1” 属性2=“属性值2”……>内容部分</标签名>     :闭合标签
<标签名 属性1=“属性值1” 属性2=“属性值2”…… />				    ：自闭合标签
```
...
> 标签大致可分为两类
>
> *  块级标签(block)  -- 独占一行               （块级标签可以嵌套块级和内联） 可以设置宽高属性
> *  内联标签(inline)  -- 按文本内容占位   （内联原则上只可以嵌套内联）

* div和span标签

````html
<div>只是一个块级元素，并无实际的意义。主要通过CSS样式为其赋予不同的表现.       无样式
<span>表示了内联行(行内元素),并无实际的意义,主要通过CSS样式为其赋予不同的表现	无样式
````
...
* 表单域，包含了文本框(text)、密码框(password)、隐藏域(hidden)、多行文本框(textarea_)、复选框(checkbox)、单选框(radio)、下拉选择框(select_option)和文件上传框(file)等表单控件。
...
| target  | 参考超链接的target属性（是否新打开一页）                     | 表单数据提交时URL的打开方式   |
| enctype | application/x-www-form-urlencoded[默认值a=b&c=d]                                                                                                                       json[json格式上传数据{a:a, b:b}]                                                                                                                                                                       multipart/form-data [用于文件上传]                                                                                                                                                               text/plain [用于纯文本数据发送] | 表单提交数据时的编码方式      |
...
CSS样式有三种不同的使用方式，分别是行内样式（行内），嵌入样式（head后面加style标签）以及链接式（link(href='style.css' )）。我们需要根据不同的场合不同的需求来使用不同的样式。
...
<div class="c1 c2">item4</div>     #可以同时有用多个class值，保存多个class的样式
...
E[att~=val]     匹配所有att属性具有多个空格分隔的值、其中一个值等于“val”的E元素。类似class='v1 v2'选取其中的一种属性
...
display: inline-block; 	 // 声明当前元素的显示模式为行内块状元素   让多个可以设置长宽的元素（块级标签）在一行显示
...
> 当一个元素被设置浮动后，将具有以下特性：
>
> 1. 任何申明为float 的元素都会自动被设置为一个行内块状元素，具有行内块状元素（inline-block）的特性。 
...
| left  | 在左侧不允许浮动元素。                                       |
| right | 在右侧不允许浮动元素。                                       |
| both  | 在左右两侧均不允许浮动元素。   ！！！！！容易出现坑，元素只能影响自己的位置，不会影响后面元素的排布                                                             【 块1、2，3 左浮动，块2   clear:both   此时块3仍然会出现在块2的后面，因为块2的样式只影响到自己，不影响块3】 |
...
（1）当声明的变量**未初始化**时，该变量的默认值是 undefined。

（2）当函数**无明确返回值**时，返回的也是值 undefined;

* null类型

另一种只有一个值的类型是 null，它只有一个专用值 null，即它的字面量。值 undefined 实际上是从值 null 派生来的，因此 ECMAScript 把它们定义为相等的。

尽管这两个值相等，但它们的含义不同。undefined 是声明了变量但未对其初始化时赋予该变量的值，null 则用于表示**尚未存在**的对象。如果函数或方法要返回的是对象，那么找不到该对象时，返回的通常是 null。
...
var box9 = [];
console.log( Boolean(box9) ); // !!!!true  此处与python不同
var box10 = {};
console.log( Boolean(box10) ); // !!!!!true 此处与python不同
var box11 = "0";
...
//算术运算符
   ...
   *   数值相乘
   ...
   %   数值求余
   **  数值求幂
   a++ 数值后自增1 先赋值再计算  a=a+1
   ++a 数值前自增1 先计算在赋值  a=a+1
   b-- 数值后自减1   b=b-1
   --b 数值前自减1   b=b-1
...
// isArray()  判断变量的值是否是数组   使用方法Array.isArray(arr)
...
```js
 for (var attr in person){
           console.log(attr,person[attr]);     // 不能用person.attr
      }
```
...
add(1,2)				
add(1,2,3)				//形式参数和实际参数可以个数不一致
add(1)					//此时b的类型为undifined
...
> 使用匿名函数表达式时，函数的调用语句，必须放在函数声明语句之**后**！
...
2. 找**变量声明**，将变量名作为GO属性名，**值为undefined**。
3. 找**函数声明**，作为GO属性，值赋予函数体.

**局部预编译的4个步骤：**

1. 创建AO对象（Activation Object）执行期上下文。（一个函数一个AO对象，但是本质上只有一个AO对象，因为一次只执行一个函数）
2. 找形参和变量**声明**，将变量和形参名作为AO属性名，**值为undefined**。（不找函数声明）
3. 将实参值和形参统一。
...
```js 
//GO：全局预编译  AO：局部预编译    z：正式执行


<script>
        var a = 10;				//GO1.1 (a=undifined)	GO2.1 (a=10)
        console.log(a);         //																						z1  10

        function foo(a) {		//						GO2.2 (foo函数)
          console.log(a);		//																					  z2  函数体
          var a = 100;			//								AO1.1 (a=undifined)   AO2.1 (a=20 从下面的函数调用中传入)
          console.log(a);		//																						z3  100
          function a() {}		//																	AO3.1 (a函数)
          console.log(a);		//																						z4  100
          var b = function(){};	//								AO1.2 (b=undifined)
          console.log(b);		//																				   		z5  b函数
          function d() {}		//																	AO3.2 (d函数)
        }
        var c = function (){	//GO1.2 (c=undifined)   GO2.3(c函数)
        console.log("匿名函数C");
        };
        console.log(c);			//																				  z6   "匿名函数C"
        foo(20);
...
```js
    ...
    GO/window = {
        ...
        foo: function(a) {
            ...
            var a = 123;
            console.log(a);
            function a() {}			//声明，正式执行时候不运行，只在预编译时候开辟空间
            console.log(a);
            var b = function() {}	
            console.log(b);
            function d() {}			//声明，正式运行时候不运行，只在预编译时候开辟空间
        }
    }
```
...
var num3 = 10;		   //GO num3=undifined  num3=10  
function func3(){	   //GO 				func函数
    console.log(num3); //	z1 undifined						 
    var num3 = 20;     //							 AO num3= undifined	  !!num3=undifined(从实参中传入)  另外虽然此处又定义了num3的值，																								   但是，是局部变量，影响不到外部的情况。
}
func3();			   
console.log(num3); 	   //	z2 10
...
// 设置循环定时器
var ID = window.setInterval(code,millisec)   // 每millisec毫秒执行一次code  ,先等待后运行
// 取消循环定时器
...
<script>
    ...
    function func2(){
        // location.href="http://www.baidu.com"; // 页面跳转,重新赋值后默认直接跳转
...
```js
document.getElementsByTagName("标签名")     //拿到的是dom数组
document.getElementById("id值")			  //拿到的是dom对象
document.getElementsByClassName("类名")	 //拿到的是dom数组
```
...
<script>
    ...
    for(var i=0;i<eles.length;i++){
        eles[i].onclick = function (){
            console.log(this.innerHTML)       // 获取当前标签的文本内容
            // console.log(eles[i].innerHTML)  // 结果，每次运行都知识返回一个执行对象，全部执行完毕后i=3，所以后面无论点击哪个标签后，都会执行																								eles[3],超过数组长度，会直接报错
        }
    }
...
<script>
    ...
    console.log(ele1.value);							// 拿input标签里面的value值
...
<script>
    var ele = document.getElementById("i1");
    ele.onclick = function (){
        this.style.color = "red"   // this.style.属性名
    }
</script>
```
...
// 插入节点
somenode.appendChild(newnode)             // 追加一个子节点（作为最后的子节点）最小的儿子
somenode.insertBefore(newnode,某个节点)    // 把增加的节点放到某个节点的前边，指定节点的位置
// 删除节点
...
* **onload事件**       ： 加载事件（先加载完毕，再出发里面的代码，防止出现先执行js未执行html时候的报错）
...
* **onsubmit事件**	：提交事件
  * 检查标签，组织不合法执行：return false;        or       function(e){ 判断条件       e.preventDefault()  };
...
<script>
     ...
     ele.onsubmit = function (e){
           ...
           console.log(pwd.value);
            ...
            return false;    // 终止事件执行
           // e.preventDefault() // 阻止元素默认行为，用到这里的时候需要再function中定义形式参数e
     }
...
<script type="text/javascript">
    ...
    var ele=document.getElementById("t1");
     ...
     ele.onkeydown=function(e){
        console.log("onkeydown",e.key)   // e就是按下的那个键
    };
...
* onblur和onfocus事件	：获取和失去焦点事件（场景：注册时候，输入用户名后，出来输入框提示用户名已存在，涉及ajax请求）
...
<script>
       ...
       // 如何将jQury对象转换为Dom对象:    直接选中序列里的元素即可
       console.log($(".c1 li")[1].innerHTML);

       // 如何将Dom对象转换为jQuery对象;   $(dom)
...
// 查找子代标签：         
 ...
 $("div").find(".test")  //所有后代，必须加条件，不然返回0
                               
 // 向下查找兄弟标签 
$(".test").next()     //下一个兄弟          
$(".test").nextAll()     //下面所有的兄弟
$(".test").nextUntil() 		//到某个标签位置，不包括最下面的条件标签
...
// 查找父标签：         
$(".test").parent()              
$(".test").parents()     //所有的前代一直到body，html
$(".test").parentUntil() 
...
三种用法:
  ...
  3. 组合事件,模拟事件
  	 $().ready(匿名函数)   // 入口函数 相当于JS的onload函数
...
//读取属性值
	//表单和非表单操作有区别的
	$("选择符").attr("属性名");   // 获取非表单元素的属性值,只会提取第一个元素的属性值，拿表单的时候，如果属性未设置，那么返回undifined
	$("选择符").prop("属性名");   // 表单元素的属性,只会提取第一个元素的属性值，拿表单的时候，如果属性值未设置，那么返回布尔值。
...
<script>
    ...
    $(".reverse").click(function () {
       $("table input:checkbox").each(function () {   //jquery的each循环
...
* css样式操作
  ...
  **样式属性要变化，background-color 要变成 backgroundColor**
...
//内部插入（子代）
    ...
    $("").append(content|fn)      // $("p").append("<b>Hello</b>");         A.append(b),A添加子节点b
    $("").appendTo(content)       // $("p").appendTo("div");				b.append(A),b添加到A后面
...
<script>
   ...
   $(".add").click(function () {
       ...
       var $clone=$(this).parent().clone()  #此处容易出现坑
    ...
    // 事件委派！！！！！！！！！！！！！！！！！让后加入的节点也拥有事件效果   此时的this对象是指派的小弟
...
```js
/*
$("").height([val|fn])  # 内容区的宽高
$("").width([val|fn])
$("").innerHeight()   # 加上padding的宽高
$("").innerWidth()
$("").outerHeight([soptions])  # 加上padding和margin的宽高
$("").outerWidth([options])
*/
```
...
```js
/*
$("").offset([coordinates])  // 获取匹配元素在当前视口的相对偏移。 返回两个属性，（offset.top, offset.left）
$("").position()             // 获取匹配元素相对父元素(已经定位的父元素)的偏移，position()函数无法用于设置操作。
...
<script>
    jQuery.extend({
      min: function(a, b) { return a < b ? a : b; },    // 三元运算符-
...
* 下载地址: http://v3.bootcss.com/getting-started/#download
  ...
  ```html
  引入bootscript的css样式
  !!!! 使用bootstrap的js样式的时候也要##！！先先先先先！！##引入jquery
