...
- `t.setDaemon(布尔值)` ，守护线程（必须放在start之前）
  ...
  - `t.setDaemon(True)`，设置为守护线程，主线程执行完毕后，子线程也自动关闭。主进程内部无法再创建子进程，会报错`AssertionError: daemonic processes are not allowed to have children`
  - `t.setDaemon(False)`，设置为非守护线程，主线程等待子线程，子线程执行完毕后，主线程才结束。（默认）

  ![image-20211212211635280](E:\home\笔记\assets\QQ截图20211212212730.png)

  ![image-20211212212804944](E:\home\笔记\assets\QQ图片20211212211535.png)

  ```python
  详细请查看：https://www.cnblogs.com/strive-man/p/8678096.html
