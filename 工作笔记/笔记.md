...
# 2，删除数据库所有表
# 3，重新迁移命令
python manage.py makemigrations --empty + app名称   # 清空迁移数据库
python manage.py makemigrations  # 迁移app数据库
```
...
def resolve_ordered_project_code_query():
    projects = Project.objects.all()
    res = []
    for project in projects:
        data = salmon_client.query_info_by_code(project.code)
        if data["runningTowerCrane"] + data["towerCraneCount"] + data["violationCount"] + data["warningCount"] != 0:
            res.append(project)
    return res


setattr(ProjectSortQuery, "sorted_project_code_query", graphene.List(
    ProjectType,
    description="有序项目code",
))
setattr(ProjectSortQuery, f"resolve_sorted_project_code_query", resolve_ordered_project_code_query())
...
__all__ = (
    ...
    def query_info_by_code(self, project_code):
        query_body = {
            "query": """
                    {
                        towerCraneSummaryQuery{
                            runningTowerCrane
                            towerCraneCount
                            violationCount
                            warningCount
        ...
        data = self.graphql.query(project_code, query_body)
        return data["data"]["towerCraneSummaryQuery"]


salmon_client = SalmonClient()
