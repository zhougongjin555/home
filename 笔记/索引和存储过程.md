...
SELECT
    *
FROM
    (SELECT nid,name FROM tb1 WHERE nid > 2) AS A -- 此处可以创建视图，用来临时替代一大串sql查询结果
WHERE
    A.name > 'alex';
```
...
- 在 t1 表中插入数据之前，先在 t2 表中插入一行数据。
  ...
  ```sql
  delimiter $$
  CREATE TRIGGER tri_before_insert_t1 BEFORE INSERT ON t1 FOR EACH ROW  -- 关键词 NEW 代表新插入的一行数据
  BEGIN
...
- 在t1表中删除数据之后，再在t2表中插入一行数据。
  ...
  ```sql
  delimiter $$
  CREATE TRIGGER tri_after_insert_t1 AFTER DELETE ON t1 FOR EACH ROW  -- OLD 代表刚刚删除的一行数据
  BEGIN
