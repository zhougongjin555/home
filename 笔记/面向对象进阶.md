...
```python
class Person(object):
    country = "中国"  # 类变量，仅属于类，通过Person.country调用
...
p1.country = "china" # 在对象p1中新增实例变量 country="china"   ！！！实例变量不会影响到类变量
...
Person.country = "美国"       # ！！！！修改的类变量


p1 = Person("武沛齐",20)
print(p1.name) # 武沛齐
print(p1.age) # 20
print(p1.country) # 美国 先查找实例变量，没找到去类变量找
```
...
- @classmethod ：类方法，当前函数使用到了类 ，可以使用类方法，将func(self, *args, ******kargs) 中的 self 改为   ‘cls‘   

- @staticmethod ：静态方法，类里面的函数没有用到类中的变量或者方法，可以使用静态方法，去掉func(self, *args, **kargs) 中的self
...
class Foo(object):
    ...
    @property   # 加上@property 再次调用后不需要使用()了
    def f2(self):
        return 123
...
特别提醒：父类中的私有成员，子类无法继承。（私生子类不允许被出自身外的其他任何类调用）
...
__init__
__new__
__call__   		通过实例化对象直接调用
__str__    		有这个对象的时候，print(实例化对象)时候输出字符串
__dict__		有这个对象的时候，print(实例化对象)时候输出字典
__getitem__		字典读取
__setitem__		字典赋值
__delitem__		字典删除
__enter__  		面向对象的  with  方法
__exit__		面向对象的  with  方法
__add__			支持对象相加
__iter__		
'''
```
...
- `__iter__`
  ...
  - 可迭代对象
    ...
    ```python
    # 基于可迭代对象&迭代器实现：自定义range   可迭代对象被  __iter__   调用后就是迭代器
...
1. @staticmethod 和 @classmethod的作用是什么？# @s 静态方法 为用到类中的变量和方法 del self @c 类方法 调用类 self--> cls
...
4. 简述你理解的：迭代器、生成器、可迭代对象。 # yeild 生成器   可迭代对象经过`__iter__` 后会变成迭代器  
...
11. 看代码写结果
    ...
    for item in data_list:
        print(item.age, item.display())   # display()会执行方法，由于无输出值，所以返回none
...
20. 分析代码关系，并写出正确的输出结果。
    ...
    print(root.title) # 河北省
    print(root[0])  # 中国
    print(root[0].title) # 河北省
    print(root[1]) # 河南省
